...
def display_out(json_obj):
    ...
    for idx, val in enumerate(list(json_obj['recipe-ids'].keys())):
        if(len(json_obj['recipe-ids'][val]['recipe_name'])>2):
            print(str(count+1) +". " + json_obj['recipe-ids'][val]['recipe_name'] + " C:\\Users\\palad\\OneDrive\\Desktop\\ProjectVed\\demo_RecipeQA\\demoRecipeQA\\data" + json_obj['recipe-ids'][val]['instructions'][-1]['modality']['image'][0][1:].replace("/","\\"))
            count = count +1 
            if(count == 6):
                break
